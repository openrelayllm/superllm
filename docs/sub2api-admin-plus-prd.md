...
用于在供应商父级下面管理“供应商分组 -> 第三方 Key -> 本地 Sub2API 账号”的落地关系。MVP 主流程不是先手工选择已有本地账号，而是在供应商分组弹窗内对某个真实分组创建第三方 Key，并同步创建本地 Sub2API `accounts.id`；切换建议、成本核算、余额、健康指标和对账最终都要落到这些绑定子级上。
...
- 调度建议时能定位到应调整的本地账号。
- 成本、余额、健康和并发指标可以按供应商父级聚合，也可以按子账号/Key 精确决策。
...
erDiagram
  ...
  admin_plus_suppliers ||--o{ admin_plus_promotion_events : has
