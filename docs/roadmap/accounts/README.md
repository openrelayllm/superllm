# 供应商分组、第三方密钥、本地账号与成本对账方案

版本：v0.2.0
日期：2026-06-21
状态：账号链路与供应商成本目标方案
范围：重新梳理供应商、后端直登、Chrome 插件兜底、第三方分组、第三方密钥、本地 Sub2API 账号、供应商充值订单、供应商兑换记录和 Admin Plus 成本对账关系。
...
10. 后续费率、余额、健康、用量账单、充值订单、兑换记录、成本台账和动作建议全部由 Provider Adapter 采集并落到供应商父级或绑定子级。
...
- 供应商成本对账分两种口径：请求利润口径使用本地收入减供应商用量消耗；资金库存口径使用期初余额、充值、兑换、退款、消耗和期末余额核对。
...
| 供应商用量账单 | 供应商 | Provider Adapter 读取供应商用户侧用量 | 是 | 对应 `/api/v1/usage`，只表示请求级消耗成本，不代表完整资金投入 |
| 供应商成本台账 | Admin Plus | 充值订单、兑换记录、用量账单、余额快照归一化生成 | 是 | 财务对账 current 事实源，用于供应商列表、成本模块和利润测算 |
...
sequenceDiagram
  ...
  O->>W: 在供应商分组弹窗点击某行开通
  W->>A: 提交创建密钥和本地账号请求
  A->>D: 检查该分组是否已有 bound/provisioning Key
  alt 已存在可用绑定
    D-->>A: 返回现有第三方 Key 和本地账号绑定
    A-->>W: 幂等返回现有绑定，不创建新 Key
  else 不存在绑定
    A->>D: 创建 supplier_key 记录，状态 provisioning
    A->>S: Provider Adapter 使用已保存统一会话探测 can_create_key
  end
...
以下流程都遵守同一个边界：Admin Plus 后端优先通过 Provider Adapter 直登供应商并生成统一会话；Chrome 插件只在验证码、2FA、强风控或浏览器上下文绑定场景下采集已登录浏览器会话。Provider Adapter 才负责读取分组、余额、费率、公告、健康、并发、用量账单、充值订单、兑换记录和创建第三方密钥。
...
| `can_read_billing` | `/api/v1/usage?page=1&page_size=1` | 判断是否可读取用户侧用量/账单 |
| `can_read_recharge_orders` | `/api/v1/payment/orders/my?page=1&page_size=1` | 判断是否可读取当前供应商下游账号的充值订单 |
| `can_read_redeem_records` | `/api/v1/redeem/history` | 判断是否可读取当前供应商下游账号的兑换记录 |
...
本节只处理供应商请求级用量账单。它是“消耗成本明细”，不是完整成本事实源。完整成本必须进入 `9.8 获取充值订单与兑换记录流程` 和 `9.9 成本台账与对账流程`。

```mermaid
flowchart TD
  A[触发每日用量账单采集] --> B{供应商会话或只读数据源是否可用}
  B -- 否 --> B1[优先后端直登，失败后记录 session_required 或 datasource_required]
  B -- 是 --> C[调用 POST /suppliers/:id/billing/sync]
  C --> D[Provider Adapter 执行 ReadBilling(session, date_range)]
  D --> E{是否读取到账单明细}
  E -- 否 --> E1[记录 capability_missing: billing]
  E -- 是 --> F[优先调用供应商用户侧 /api/v1/usage]
  F --> G[归一化 request_id/model/tokens/cost/计费模式/时间]
  G --> H[写入 supplier_bill_lines]
...
flowchart TD
  ...
  D -- 否 --> D1[记录 capability_missing: recharge_orders]
  ...
  I -- 否 --> I1[记录 capability_missing: redeem_records]
...
- `ReadRechargeOrders(session, date_range)` 对 Sub2API 同源供应商调用 `/api/v1/payment/orders/my`，仅保存当前下游账号可见订单。
- `ReadRedeemRecords(session, date_range)` 对 Sub2API 同源供应商调用 `/api/v1/redeem/history`，仅保存当前下游账号已兑换记录。
...
成本模块的 `current` 事实源是 `admin_plus_supplier_cost_ledger_entries` 和按供应商聚合的成本快照。充值订单、兑换记录、供应商用量账单和余额快照只作为生成台账的输入。

```mermaid
flowchart TD
  A[充值订单/兑换记录/用量账单/余额快照入库] --> B[生成供应商成本台账分录]
  ...
  E -- 否 --> G[生成供应商成本异常]
  B --> H[请求级对账]
  H --> I[本地 usage_logs 收入 - supplier_bill_lines 消耗]
  I --> J[计算毛利和毛利率]
```
...
- `current`：`supplier_cost_ledger_entries`、`supplier_cost_snapshots`、`supplier_recharge_orders`、`supplier_redeem_records`。
- `current` 但边界受限：`supplier_bill_lines` 只用于请求级消耗成本，不再单独驱动财务成本汇总。
- `compat`：手工补录账单、旧 `export_bills` 和历史 `billing` 页面导入能力，只能生成台账分录或委托新成本服务，不能新增独立成本计算。
...
flowchart TD
  ...
  H -- 否 --> J[记录 failed + LOCAL_ACCOUNT_CREATE_FAILED]
  J --> K[人工补本地账号并执行 repair-binding]
  I --> L{绑定创建成功}
  L -- 是 --> M[完成]
  L -- 否 --> N[记录 failed + SUPPLIER_ACCOUNT_BIND_FAILED]
  N --> O[选择已有本地账号执行 repair-binding]
  O --> M
```
...
- 第三方密钥创建成功、本地账号创建失败：记录失败 Key 元数据和错误码，管理员手动补本地账号后通过 `repair-binding` 绑定，不让插件参与业务补偿。
- 本地账号创建成功、绑定失败：不重复创建第三方 Key，选择已有本地账号通过 `repair-binding` 补绑定。
...
- 提供“同步成本”动作：一次触发充值订单、兑换记录、用量账单和余额快照同步，再刷新成本快照。
...
- “只同步用量”：保留当前 `billing/sync` 的能力，但结果只进入请求利润对账。
...
- `供应商账单` tab 改名为 `用量消耗`。
- `成本` 卡片改名为 `用量消耗成本`。
- 页面结果必须从成本模块读取汇总或跳转到成本模块，不允许继续独立计算供应商总成本。
...
backend/internal/adminplus/app/costs/
  ...
  sql_repository.go              # supplier_recharge_orders / supplier_redeem_records / cost_ledger / cost_snapshots
  memory_repository.go           # 单元测试仓储
  provider.go                    # Wire provider
...
- Provider Adapter 是采集主路径，负责分组、费率、余额、公告、用量账单、充值订单、兑换记录、健康、并发和第三方密钥创建。
- Provider Adapter 同时负责可自动化的供应商后端直登。
- Chrome 插件只负责在直登不可用时识别站点、采集已登录浏览器会话、采集页面上下文和上报第三方供应商信息；不承载分组解析、密钥创建、费率、余额、用量账单、充值订单、兑换记录等业务动作。
...
### 13.4 `admin_plus_supplier_recharge_orders`
...
### 13.5 `admin_plus_supplier_redeem_records`
...
当前已落地的是同步返回的基础链路。第三方 Key 明文只用于创建本地 Sub2API 账号，不在接口响应、`provision_response` 或列表接口中回显。`POST /api/v1/admin-plus/suppliers/:id/keys/provision` 已接入 `Idempotency-Key` 去重和结果重放；未携带 key 的旧客户端仍依赖供应商分组唯一约束兜底。
...
```json
{
  ...
  "include_recharge_orders": true,
  "include_redeem_records": true,
  "include_usage_billing": true,
...
```json
{
  ...
  "capabilities": {
    "can_read_recharge_orders": true,
    "can_read_redeem_records": true,
...
插件不通过该链路上报已经解析好的分组、费率、余额、用量账单、充值订单、兑换记录或第三方密钥创建结果。
...
- 供应商管理页的“分组”弹窗已作为 Key/账号开通和失败修复主入口：每个分组行展示绑定状态，未绑定行可触发 `keys/provision`，本地落地失败行可触发 `repair-binding`。

待实现并作为成本 current 主链路：

- 通过 `POST /api/v1/admin-plus/suppliers/:id/costs/sync` 读取供应商用户侧 `/api/v1/payment/orders/my` 和 `/api/v1/redeem/history`。
- 把供应商充值订单写入 `admin_plus_supplier_recharge_orders`。
- 把供应商兑换记录写入 `admin_plus_supplier_redeem_records`。
...
当前 `keys/provision` 和 `keys/:keyID/repair-binding` 已接入通用 `idempotency_records`：
