...
> 关系图事实源更新：供应商管理、账号管理、第三方令牌分组管理、检测同步和路由补池的最新全局关系以 [`../supplier-architecture/README.md`](../supplier-architecture/README.md) 为准。本文保留账号开通链路和异步治理历史上下文。
>
> 数据库设计事实源更新：Admin Plus 表域划分、ER 图、导入导出边界、敏感字段和流程表级数据流转以 [`../supplier-architecture/08-database-design.md`](../supplier-architecture/08-database-design.md) 为准。本文中的表结构描述若与该文档冲突，以数据库设计文档为准。
>
...
>
> 账号开通异步治理：分组同步、第三方 Key 创建、真实 Sub2API 账号落地和绑定投影的异步 job / outbox / worker / Saga 方案见 [`ASYNC_PROVISIONING.md`](ASYNC_PROVISIONING.md)。其中关于 `Sub2APIHTTPGateway` 和嵌入式回退的描述属于历史实施记录；新的系统边界以 Admin Plus 本地 Sub2API 端口为准，本地同进程优先复用现有 service，远程实例才配置现有 Sub2API Admin API。
...
8. Admin Plus 提交异步开通任务，由 Worker 通过真实 Sub2API Admin API 创建或修正本地账号。
9. Admin Plus 建立供应商父级、第三方密钥、本地 Sub2API 账号之间的绑定投影。
...
- 写本地 Sub2API 调度状态必须走 Admin Plus 本地 Sub2API 端口，端口内部优先复用现有 service 并触发 `scheduler_outbox`；远程实例才调用现有 Sub2API Admin API。读取可以走 service、Admin API、只读 DB 或只读 Redis。
...
sequenceDiagram
  ...
  alt 已拿到密钥明文
    Q->>L: ensure 真实 Sub2API account，写入 secret
...
健康、并发和账号运行态是供应商状态、账号/Key 绑定和动作建议的输入事实，不注册独立后台页面。后台只在供应商、绑定、成本对账和运营事件等主流程中展示必要字段和事件；公告 current 入口统一为 `/admin/events/announcements`，旧 `/admin/monitoring/*` 仅作为 compat 重定向，不再承载 Admin Plus 运维监控页面。
...
flowchart TD
  ...
  G --> H{真实 Sub2API account ensure 成功}
...
- 第三方密钥创建成功、真实 Sub2API account ensure 失败：记录失败 Key 元数据和错误码，只重试 Sub2API 落地或进入人工修复，不让插件参与业务补偿。
- 真实 Sub2API account 创建成功、绑定投影失败：不重复创建第三方 Key，只重试 Admin Plus 绑定投影。
...
8. Worker 通过真实 Sub2API Admin API ensure account。
...
- job 进入 `succeeded`、Admin Plus 绑定投影存在、真实 Sub2API Admin API 可查询到 account 后，才能展示“开通完成”。
...
- `admin_plus_supplier_accounts`：Admin Plus 绑定投影，保留，必须追溯真实 Sub2API account。
...
- 当前 `POST /api/v1/admin-plus/suppliers/:id/keys/provision` 已有同步基础链路，但它是 `deprecated` 过渡实现；目标链路必须改为提交 `provision_group_key` job，由 Worker 创建第三方 Key、ensure 真实 Sub2API account 并写入绑定投影。
