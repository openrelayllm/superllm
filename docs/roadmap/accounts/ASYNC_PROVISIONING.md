...
| `compat` | `UseSub2APIGateway(adminService)` 的本进程回退实现 | 仅在显式设置 `ADMIN_PLUS_ALLOW_EMBEDDED_SUB2API_GATEWAY=true` 时用于本地开发；生产不得启用 |
...
- Saga 用于把“创建第三方 Key -> 创建真实 Sub2API account -> 写绑定”拆成可重试、可补偿的本地事务。
...
flowchart TD
  ...
  GA -. config missing / invalid .-> FAIL[Fail Fast: SUB2API_GATEWAY_CONFIG_REQUIRED]
  ORCH --> DB
...
| Sub2API Gateway Adapter Layer | 通过真实 Sub2API Admin API ensure account | 不直写 Sub2API DB/Redis |
...
`provision_all_group_keys` 不允许一次性在请求线程内循环创建。它只创建一个 batch job，然后为每个 active 供应商分组创建 `ensure_third_party_key` step；已有绑定的分组在 step 内走幂等校验和本地 Sub2API account 修正，不重复创建第三方 Key。
...
flowchart TD
  ...
  G --> H{分组已有 bound key?}
  H -- 是 --> I[修正 Sub2API account 绑定并标记 skipped]
...
- 本地 Sub2API account 创建是 retryable step；重试前必须先查找是否已经创建。
...
5. 上线后用治理扫描或测试禁止生产 ProviderSet 回退到 Admin Plus 本地 `AdminService` 作为真实 Sub2API 写入路径；生产必须配置 Sub2API Gateway HTTP Adapter 调真实 Sub2API Admin API。当前已改为默认 fail-fast，只有显式设置 `ADMIN_PLUS_ALLOW_EMBEDDED_SUB2API_GATEWAY=true` 才允许本地开发回退。
...
# 仅本地开发兼容使用；生产不得启用
ADMIN_PLUS_ALLOW_EMBEDDED_SUB2API_GATEWAY=false
```

运行规则：

- 两个环境变量同时存在时，`UseSub2APIGateway` 走真实 Sub2API Admin API。
- 任一变量缺失或 URL 非法时，默认返回 `FailingSub2APIGateway`，开通任务失败并暴露 `SUB2API_GATEWAY_CONFIG_REQUIRED` 或 `SUB2API_GATEWAY_BASE_URL_INVALID`，禁止继续写 Admin Plus 本地投影制造假同步。
- 只有显式设置 `ADMIN_PLUS_ALLOW_EMBEDDED_SUB2API_GATEWAY=true` 时，才回退同进程 `AdminService`，该路径仅用于本地开发和旧测试兼容。
...
如果 Admin Plus 的“账号/Key 绑定”已有记录，但真实 Sub2API `/admin/accounts` 看不到账号，按以下顺序判定：

```mermaid
flowchart TD
  A[Admin Plus 绑定存在] --> B{真实 Sub2API 账号可见?}
  B -->|是| C[链路已打通]
  B -->|否| D{Admin Plus 是否配置 HTTP Gateway?}
  D -->|否| E[配置 ADMIN_PLUS_SUB2API_ADMIN_BASE_URL / API_KEY]
  D -->|是| F{任务 step 是否失败?}
  F -->|是| G[按 step error_code 修复并重试]
  F -->|否| H[检查是否曾启用 embedded fallback 生成历史投影]
  E --> I[重新触发 ensure-all / repair-binding]
  G --> I
  H --> I
  I --> J[Worker 通过真实 Sub2API Admin API ensure account]
```

治理结论：

- `current`：真实 Sub2API Admin API 中的 group/account。
- `current`：`admin_plus_supplier_keys`、`admin_plus_supplier_accounts` 作为运营投影和审计索引。
- `compat`：同进程 `AdminService` fallback，仅本地开发显式开关可用。
- `dead`：生产环境无配置时静默 fallback 并标记绑定成功。
...
1. 第三方 Key 投影为 `bound`。
2. Admin Plus 绑定投影存在。
3. 真实 Sub2API Admin API 查询到 account。
...
| E2E | 添加供应商、会话获取、同步 11 个分组、每组一个 Key、Admin Plus 可见绑定、真实 Sub2API 可见 account |
...
10. [x] 新增可配置 `Sub2APIHTTPGateway`，配置 `ADMIN_PLUS_SUB2API_ADMIN_BASE_URL` 和 `ADMIN_PLUS_SUB2API_ADMIN_API_KEY` 后通过真实 Sub2API Admin API ensure account。
11. [x] 将生产默认静默回退同进程 `AdminService` 改为 `FailingSub2APIGateway`，配置缺失或非法时显式失败；仅 `ADMIN_PLUS_ALLOW_EMBEDDED_SUB2API_GATEWAY=true` 保留本地开发 compat。
12. [x] 将 `provision_all_group_keys` 从单 step `EnsureAll` 拆成每分组 `ensure_third_party_key` step，支持部分成功和分组级重试。
13. [ ] 增加真实 Sub2API 可见性验证和 E2E。
14. [ ] 将旧插件业务任务降级为 compat 补录入口，默认任务中心只保留会话采集和后端任务状态。
...
| gateway boundary | `backend/internal/adminplus/app/supplierkeys/service.go`、`provider.go`、`sub2api_gateway_http.go`、`sub2api_gateway_failing.go` | 已将本地 Sub2API 落地语义收口为 `Sub2APIGateway`；已新增可配置 HTTP Adapter 和 fail-fast gateway；`LocalAccountService` 仅为兼容别名 |
