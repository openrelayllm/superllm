# Chrome 插件路线图：供应商浏览器会话兜底桥

版本：v0.1.0
日期：2026-06-20
状态：当前 Chrome 插件设计事实源
范围：纠正旧版“插件承担业务采集主路径”的设计偏差，将插件收敛为后端直登失败时的浏览器会话兜底桥梁。
...
Admin Plus 需要从第三方供应商后台获取费率、余额、公告、账单和运行状态。旧版设计把 Chrome 插件设计成“登录 + 页面抓取 + CSV 导出 + 结构化业务结果上传”的通用执行器，容易导致以下问题：

- 插件内业务逻辑过多，供应商适配散落在浏览器脚本中。
- DOM 抓取脆弱，供应商页面升级后容易失效。
- 费率、余额、账单、公告的归一化规则可能在后端和插件中重复实现。
- 插件需要理解业务含义，违反单一职责。
...
```text
Admin Plus 后端直登优先：Provider Adapter 调用供应商登录 API，获取用户侧会话。
Chrome 插件兜底：仅在验证码、2FA、强风控或浏览器上下文绑定时，采集已登录浏览器会话。
Admin Plus Provider Adapter 负责分组、费率、余额、公告、健康、并发、账单和密钥创建。
```

Chrome 插件仍然重要，但它不是默认登录器，也不是业务采集器。它的价值是把“必须由浏览器完成”的人工登录态安全、可审计地交给后端。

## 2. 设计结论

会话获取主路径：

1. Admin Plus 后端优先通过 Provider Adapter 模拟登录供应商。
2. 无验证码、无 2FA、无强风控时，后端直接保存 `direct_login` 会话包。
3. 后端直登失败且原因是验证码、2FA、强风控、浏览器指纹绑定或未知页面上下文时，才提示使用 Chrome 插件。
4. Chrome 插件通过 sub2apiplus 授权后，识别当前供应商页面，采集已登录浏览器会话和页面上下文。
5. 后端把 `direct_login` 和 `browser_extension` 都归一化为统一供应商会话包。
6. 后续分组、费率、余额、公告、账单、健康和创建第三方 Key 全部由 Provider Adapter 基于统一会话执行。
...
6. 后端使用会话包通过 Provider Adapter 模拟已登录 API 请求，完成会话探测和业务采集。

插件不承担业务兜底执行：

1. 插件不解析分组、费率、余额、公告、健康、并发或账单。
2. 插件不创建第三方密钥。
...
- KISS：后端能直登就不打开插件；插件只做浏览器必须做的事，后端做业务。
- YAGNI：不为每个供应商提前写复杂 DOM 解析器。
- DRY：费率、余额、账单、公告归一化只在后端 Provider Adapter 和业务服务中实现一次。
- SOLID：插件、任务协议、Provider Adapter、业务服务边界清晰。
- 稳定性更高：优先后端登录和 API，会话受限时再用浏览器兜底，而不是依赖页面 DOM。
...
- Admin Plus 后端能优先尝试供应商直登，并在失败时给出可操作原因。
- 后端直登不可用时，插件能作为兜底通道采集已登录浏览器会话。
...
- 后端能用 `direct_login` 或 `browser_extension` 统一会话包完成供应商 API 采集。
...
- 插件不作为默认供应商登录方式；能后端直登的供应商不要求打开插件。
...
flowchart LR
  ...
  Operator --> PlusWebApp[Admin Plus Web]
  Browser --> PlusWeb[sub2apiplus Web 登录/授权页]
  Browser --> SupplierWeb[供应商后台网页]
  Browser --> Extension[Chrome 插件]

  PlusWebApp --> SessionOrchestrator[会话获取编排]
  SessionOrchestrator --> DirectLogin[后端直登 Provider Adapter]
  DirectLogin --> SupplierLoginAPI[供应商登录 API]
  DirectLogin --> UnifiedSession[统一供应商会话]
  ...
  PlusAPI --> UnifiedSession
  UnifiedSession --> SecretStore
  UnifiedSession --> ProviderAdapter[Provider Adapter]
...
sequenceDiagram
  ...
  O->>W: 点击获取/刷新供应商会话
  W->>A: POST /suppliers/:id/session/login
  A->>V: 使用加密登录配置执行 direct_login
  V->>S: 调用供应商登录 API
  alt 直登成功
    S-->>V: 返回 access token/cookie/CSRF/用户上下文
    V-->>A: 返回 direct_login 会话包
    A->>A: 加密保存统一会话并探测能力
    A-->>W: 返回会话有效和能力矩阵
  else 需要验证码/2FA/浏览器环境
    S-->>V: 返回 challenge 或风控失败
    V-->>A: 返回 browser_fallback_required
    A-->>W: 提示打开 Chrome 插件兜底
    O->>X: 在供应商网页完成登录后点击一键上报
...
sequenceDiagram
  ...
  S->>A: 触发供应商采集
  A->>D: 读取统一供应商会话包和能力矩阵
  A->>V: 使用 direct_login 或 browser_extension 会话包创建采集 client
  V->>U: 调用已登录内部 API
  U-->>V: 返回费率、余额、账单或公告数据
  V->>A: 返回归一化前原始响应和解析结果
  A->>D: 写入快照、明细、事件
...
```mermaid
flowchart TD
  A[开始获取供应商会话] --> B{是否配置后端直登}
  B -->|是| C[Provider Adapter 调用登录 API]
  C --> D{直登成功}
  D -->|是| E[保存 direct_login 会话包]
  D -->|否| F{是否需要浏览器兜底}
  F -->|否| G[返回 login_failed 或 adapter_missing]
  F -->|是| H[提示使用 Chrome 插件]
  B -->|否| H
  H --> H1{插件是否已连接 sub2apiplus}
  H1 -->|否| H2[打开 sub2apiplus Web 登录/授权页]
  H1 -->|是| I[领取短期 capture_supplier_session 租约]
  I --> J{当前供应商页面是否已登录}
  J -->|否| K[提示先在供应商网页登录后刷新]
  J -->|是| L[读取浏览器上下文]
  L --> M[提取 token/cookie/CSRF/组织信息]
  M --> N[脱敏诊断信息]
  N --> O[上报 browser_extension 会话包]
  O --> P[后端加密存储]
  E --> Q[后端会话探测]
  P --> Q
  Q --> R{会话 API 是否可用}
  R -->|是| S[启用后端会话 API 采集]
  R -->|否| T[标记能力缺失]
  T --> U[提示补 Provider Adapter 或人工处理]
```
...
### 19.3 SupplierSessionProbeResult
...
- 插件兜底路径通过 `capture_supplier_session` 短租约任务上报会话包，插件完成任务时把 `session_bundle` 放在 `complete.result.session_bundle`；后端 ingest 会删除明文、加密保存，并在任务结果中只保留摘要。
- `POST /api/v1/admin-plus/suppliers/:id/browser-sessions` 已作为管理员登录态下的直接上报入口落地，用于调试、手动导入和插件联调；插件生产兜底路径仍优先使用短租约任务，避免绕过设备、租约和任务审计。
- `browser-credential` 是历史浏览器登录凭据租约读取机制；新主路径优先后端直登，插件只支持“已登录页面一键上报”兜底，供应商自动登录不是插件必须完成的业务动作。
...
- 不再要求插件完成费率、余额、并发页面抓取；插件只保留浏览器会话兜底能力。
...
- 后端可对无验证码、无 2FA 的供应商执行 direct_login。
- 后端直登失败时能返回 `browser_fallback_required`、`captcha_required`、`mfa_required` 或明确登录失败原因。
- 会话包加密存储。
- 后端可探测会话有效性。
- 后端可记录能力矩阵。
- 会话失效可生成明确事件。
