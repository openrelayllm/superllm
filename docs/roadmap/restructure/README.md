...
- 费率、余额、健康、公告、账单页面各自服务不同任务，不做重复表格换标题。
...
- 清理或降级旧插件业务任务：`fetch_groups`、`fetch_rates`、`fetch_balance`、`fetch_announcements`、`fetch_health`、`export_bills` 已由调度中心显式选择时直连后端 Provider Adapter / app service；旧插件业务结果摄取仅作为 compat 补录路径，不作为主路径验收。
...
- `ReadBilling(session, date_range)` 已落地：`POST /api/v1/admin-plus/suppliers/:id/billing/sync` 使用后端 Provider Adapter 优先读取供应商用户侧 `/api/v1/usage`，归一化后写入 `admin_plus_supplier_bill_lines`；旧插件 `export_bills` 只保留 compat。
- 调度中心已收口：默认只创建 `capture_supplier_session` 插件任务；显式选择 `fetch_groups`、`fetch_rates`、`fetch_balance`、`fetch_announcements`、`fetch_health`、`export_bills` 时直接调用后端分组、费率、余额、公告、健康、账单同步 service；旧插件结果摄取仅作为兼容入口保留。
- `CreateKey(session, group, params)` 基础链路已落地：供应商侧用户 Key 创建接口为 `/api/v1/keys`；第三方 Key 明文只在 Adapter -> Service -> 本地 Sub2API Admin API 的内存链路中流转，响应和 `provision_response` 不保存明文 key/token/secret。

下一步仍未完成：

- 真实供应商公告端点联调、健康探测生产账号验证和失败可观测完善。
