...
- P2 不能整体结束：模型级候选第一阶段、纯度检测联动/7 天过期策略/复检深链/事实源刷新/历史报告深链第一阶段、代理联动第一阶段、通知矩阵第一阶段和实测预算/冷却第一阶段已落地，成本利润看板细化、通知升级策略、代理中心深度质量联动和细粒度实测成本归集仍未完成。
...
- 已完成第一阶段：纯度检测进入候选评估，明确失败阻断、风险降级、未知不阻断，并生成纯度复核动作建议；动作建议可直接定位本地账号复检，复检成功后写回调度 step 快照并刷新候选事实源；最近报告可深链到调度中心 step 详情查看结果快照。
- 已完成第一阶段：Sub2API 账号绑定代理进入候选评估，明确代理不可用阻断并生成代理复核动作建议；无代理或 unknown 不误阻断。
- 未完成：Admin Plus 代理中心节点质量、出口失败、fallback 建议与本地账号绑定代理的深度联动；纯度检测仍需补齐按模型/能力标签的批量复检。
