...
- P2 不能整体结束：模型级候选第一阶段已落地，成本利润看板、通知矩阵、代理质量联动、纯度检测联动、实测预算/冷却仍未完成。
...
| 通知策略 | 关键风险主动提醒 | 未完成：余额、Key 配额、分组耗尽、drift、通道异常的通知矩阵和抑制策略 |
| 代理质量联动 | 区分供应商坏和网络出口坏 | 未完成：代理健康、出口失败、fallback 建议 |
| 纯度检测联动 | 避免补入能力不匹配账号 | 未完成：模型能力标签、风险等级与候选评估联动 |
| 实测预算/冷却 | 避免用主动实测替代监控 | 未完成：按供应商/模型/动作来源限制 token 实测预算和冷却 |
...
- 未完成：通知覆盖余额、配额、耗尽、drift、通道异常，并具备抑制/升级策略。
- 未完成：代理质量和纯度检测进入候选评估。
- 未完成：主动实测有预算、冷却和审计。
