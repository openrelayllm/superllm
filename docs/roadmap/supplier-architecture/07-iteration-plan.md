...
- P2 当前运营闭环第一阶段可以收口：模型级候选第一阶段、纯度检测联动/7 天过期策略/复检深链/事实源刷新/历史报告深链/当前页模型能力圈选/受控批量复检队列、代理联动第一阶段、通知矩阵第一阶段、动作建议超时未处理可视化、实测预算/冷却第一阶段、验收步骤矩阵、目标毛利缺口和建议价偏离已落地。自动升级投递、值班分组、多渠道通知、代理中心深度质量联动、跨页/后台批量复检策略、完整财务汇总和细粒度实测成本归集保留为后续 P2.x 增强，不阻断当前 P1/P2 收口。
- P3 按当前决策不实施；已完成的 `RemoteAdminAPIRoutingPort` 只作为远程写回第一阶段保留，不扩展多实例策略。

阶段关闭证据和发布前验收清单已经收口到 [09-phase-closure.md](09-phase-closure.md)。当前版本后续只做缺陷修复和验收问题修补；新增能力默认进入 P1.x/P2.x/P3 backlog，不再扩大 P1/P2 关闭范围。
...
| 成本利润看板 | 让运营知道低倍率是否真的赚钱 | 第一阶段增强已完成：模型利润行展示目标毛利缺口和建议价相对市场中位价的偏离。未完成：完整成本、收入、毛利和异常对账汇总视图 |
| 通知策略 | 关键风险主动提醒 | 第一阶段已完成：余额、健康、费率、成本沿既有通知链路投递；动作建议持久化后会按 `action.balance_required/action.key_capacity/action.routing_capacity/action.local_state/action.channel_failure/action.proxy_review/action.purity_review/action.cost_reconcile/action.profit_review` 映射到通知中心，复用规则、投递、去重和静默窗口；动作建议页已按严重/警告/普通阈值标记超时未处理建议。未完成：跨事件升级、值班分组、自动升级投递和多渠道通知 |
...
| 纯度检测联动 | 避免补入能力不匹配账号 | 第一阶段已完成：本地账号运营读模型读取同供应商、同本地账号最近 `run_purity_check` step 结果，并按固定 7 天 TTL 派生 `purity_freshness_status=fresh/stale/unknown`；`CandidateEvaluator` 对明确失败输出 `purity_failed` 并阻断，对风险态输出 `purity_risk` 并降级，对通道可用但纯度过期的候选输出 `purity_stale` 并生成复检建议，未知不阻断；本地账号运营、供应商详情和动作建议展示纯度状态；动作建议页可深链到本地账号运营并自动打开纯度复检弹窗，账号复检成功后写入 `admin_plus_scheduler_runs/steps`，候选读模型可立即读取最新纯度快照；本地账号运营页可从最近纯度报告跳转调度中心 run/step 并查看结果快照；本地账号运营页支持按当前页模型/能力标签圈选过期纯度账号，并启动受控复检队列逐个打开账号弹窗，由运营显式点击开始检测 |
...
flowchart TD
    ...
    M --> N[P2 通知矩阵第一阶段]
    N --> O[P2 当前闭环收口]
    O --> P[P2.x 成本细化/通知升级/代理深度质量/跨页复检/细粒度成本]
```
...
- 已完成第一阶段：成本利润看板展示目标毛利缺口和建议价相对市场中位价的偏离，能判断低倍率渠道是否仍有安全垫。
- 已完成第一阶段：通知覆盖余额、Key 配额、分组耗尽、drift/本地状态、通道异常、代理复核、纯度复核、成本对账和利润复核，并复用通知中心规则、投递记录、去重和静默窗口。
- 已完成第一阶段：动作建议页标记超时未处理建议，严重 2 小时、警告 12 小时、普通 24 小时后进入超时统计。
- 未完成：通知自动升级投递、值班分组和多渠道投递策略。
- 已完成第一阶段：纯度检测进入候选评估，明确失败阻断、风险降级、未知不阻断，并生成纯度复核动作建议；动作建议可直接定位本地账号复检，复检成功后写回调度 step 快照并刷新候选事实源；最近报告可深链到调度中心 step 详情查看结果快照；本地账号运营页支持按当前页模型/能力标签圈选过期账号，并进入受控批量复检队列。
- 已完成第一阶段：Sub2API 账号绑定代理进入候选评估，明确代理不可用阻断并生成代理复核动作建议；无代理或 unknown 不误阻断。
- 后续 P2.x 增强：Admin Plus 代理中心节点质量、出口失败、fallback 建议与本地账号绑定代理的深度联动；纯度检测的跨页/后台批量复检策略和按模型预算归集。
- 已完成第一阶段：主动实测有每日预算和同分组冷却；预算/冷却跳过不会误触发自动关调度。
- 已完成第一阶段：接入验收看板按验收报告汇总连通性、模型列表、纯度、轻量调用、Usage 计量、缓存审计、余额账单和并发限速的通过/警告/失败/未知计数，能定位接入阻断点。
...
| 队列 | 内容 | 处理原则 |
|------|------|----------|
| P1.x | 真实最大 Key 上限自动读取、账单明细自动定位、批量账单导入、非本地路由动作回滚 | 只在具体供应商接口或账单来源明确后实施，不阻塞 P1 发布 |
| P2.x | 通知升级、值班分组、多渠道通知、代理深度质量、跨页纯度复检、完整财务汇总、细粒度实测成本 | 作为运营增强逐项实施，不改变 P2 第一阶段关闭结论 |
| P3 | 多 Sub2API 实例、跨实例容量、迁移冲突增强、外部事件适配 | 本轮不实施，不进入当前验收 |
