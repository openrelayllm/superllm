...
| [09-phase-closure.md](09-phase-closure.md) | P1/P2 阶段收口基线、关闭证据、发布前验收清单和后续 P1.x/P2.x/P3 边界 |
...
- P2 当前运营闭环第一阶段可以收口：模型级候选第一阶段已完成，补池可按 `model_scope` 选择候选，明确不匹配输出 `model_scope_unsupported`；纯度检测联动和过期策略第一阶段已完成，本地账号运营镜像会读取最近纯度检测结果并派生 `purity_freshness_status=fresh/stale/unknown`，明确失败输出 `purity_failed`，风险态输出 `purity_risk`，过期输出 `purity_stale` 并进入复检动作建议；本地账号运营页支持按当前页模型/能力标签圈选过期纯度账号并进入受控复检队列；代理联动第一阶段已完成，候选评估会读取 Sub2API `accounts.proxy_id -> proxies`，明确代理不可用输出 `check_source=proxy`，无代理或未知不阻断；通知矩阵第一阶段已完成，动作建议会按余额、Key 配额、分组容量、drift/本地状态、通道失败、代理、纯度、成本和利润风险进入通知中心；动作建议页已提供严重/警告/普通建议的超时未处理可视化；实测预算/冷却第一阶段已完成，渠道检测可按每日 token 预算和同分组冷却跳过主动实测；接入验收看板已提供验收步骤矩阵，能按连通性、模型列表、纯度、轻量调用、Usage、缓存、余额和并发汇总阻断点；成本利润看板已展示目标毛利缺口和建议价相对市场中位价的偏离。自动升级投递、值班分组、多渠道通知、代理深度联动、跨页/后台批量复检策略、完整财务汇总和细粒度实测成本归集保留为后续 P2.x 增强，不阻断当前 P1/P2 收口。
- P3 本轮不实施：已完成的 `RemoteAdminAPIRoutingPort` 只作为远程写回第一阶段保留，不继续扩展多 Sub2API 实例、跨实例容量和迁移冲突修复。

阶段关闭证据和发布前验收清单见 [09-phase-closure.md](09-phase-closure.md)。后续如果继续做 P1.x/P2.x，必须先确认它不会重新扩大 P1/P2 的关闭标准。
