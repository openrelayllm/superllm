...
| 健康探测 | `app/health` | 主动请求验证延迟和错误，优先级最低 | 最近样本、错误分类、检测成本、实测预算 |
...
| 纯度检测 | `app/purity` | 验证账号是否符合目标模型/能力 | 待联动候选评估：检测报告、风险等级、可用性标记 |
...
| P2 | 成本和质量优化 | 模型级候选第一阶段已完成；成本看板、实测预算、纯度检测、代理质量、通知矩阵和验收看板待补齐 |
