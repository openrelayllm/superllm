...
- 纯度检测过期不会被当成供应商坏，也不会触发主动实测；当最近 `run_purity_check` 快照超过 7 天且通道监控可用时，候选输出 `purity_stale`，动作建议生成纯度复检待办。动作建议页提供“复检纯度”入口，跳转本地账号运营并自动打开对应账号的纯度弹窗；运营显式点击开始检测后，检测结果会写入新的 `run_purity_check` step，候选评估随列表刷新读取最新纯度状态。本地账号运营行提供“报告”入口，可跳转调度中心对应 run/step 并查看结果快照；当前页可一键选中过期纯度账号并启动受控复检队列，逐个打开弹窗，不自动执行 token 检测。
