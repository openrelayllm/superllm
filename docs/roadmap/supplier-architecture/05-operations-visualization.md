...
- 纯度检测过期不会被当成供应商坏，也不会触发主动实测；当最近 `run_purity_check` 快照超过 7 天且通道监控可用时，候选输出 `purity_stale`，动作建议生成纯度复检待办。动作建议页提供“复检纯度”入口，跳转本地账号运营并自动打开对应账号的纯度弹窗；运营显式点击开始检测后，检测结果会写入新的 `run_purity_check` step，候选评估随列表刷新读取最新纯度状态。本地账号运营行提供“报告”入口，可跳转调度中心对应 run/step 并查看结果快照；当前页可用“复检模型/能力”下拉按第三方分组能力、规格或最近检测模型圈选过期纯度账号，并启动受控复检队列，逐个打开弹窗，不自动执行 token 检测。
...
- 已落地的 24 小时请求、错误、上游 429、token 和失败请求摘要目前在补池 preview/apply 影响面板里展示，用于执行前确认用户影响。
- P2.x 增强：容量矩阵行内仍可补今日请求、限流账号、错误账号和最低可补倍率，让运营不必先点开补池预览即可扫全局风险；该增强不阻断当前 P1/P2 收口。
