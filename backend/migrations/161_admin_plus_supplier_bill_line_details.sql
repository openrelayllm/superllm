ALTER TABLE admin_plus_supplier_bill_lines
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'admin_plus_supplier_bill_lines_detail_check'
    ) THEN
        ALTER TABLE admin_plus_supplier_bill_lines
            ADD CONSTRAINT admin_plus_supplier_bill_lines_detail_check
            CHECK (
                cache_read_tokens >= 0
                AND total_tokens >= 0
...
UPDATE admin_plus_supplier_bill_lines
...
CREATE INDEX IF NOT EXISTS idx_admin_plus_supplier_bill_lines_model_started
    ON admin_plus_supplier_bill_lines(model, started_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_admin_plus_supplier_bill_lines_api_key_started
    ON admin_plus_supplier_bill_lines(api_key_name, started_at DESC, id DESC)
    WHERE api_key_name <> '';
