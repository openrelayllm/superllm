...
import (
	...
	adminplus2 "github.com/Wei-Shaw/sub2api/internal/adminplus"
	...
	"github.com/Wei-Shaw/sub2api/internal/adminplus/app/importexport"
	"github.com/Wei-Shaw/sub2api/internal/adminplus/app/kanban"
...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	feishuNotifier := suppliergroups.NewFeishuNotifier(notificationsService)
	...
	recorder := adminplus2.ProvideBusinessLogRecorder(opsRepository)
	sessionsService := sessions.ProvideService(sessionsSQLRepository, cipher, suppliersService, router, router, router, recorder)
	suppliergroupsService := suppliergroups.NewServiceWithNotifier(suppliergroupsSQLRepository, feishuNotifier, sessionsService, router)
	...
	costsFeishuNotifier := costs.NewFeishuNotifier(notificationsService)
	...
	balancesService := balances.ProvideService(balancesSQLRepository, balancesFeishuNotifier, sessionsService, router, sub2apiSQLRepository, balanceCache, recorder)
	costsService := costs.NewServiceWithDependenciesAndNotifier(costsSQLRepository, costsFeishuNotifier, sessionsService, router, router, usagecostsService, balancesService, suppliersService)
	...
	kanbanSQLRepository := kanban.NewSQLRepository(db)
	...
	runtimeConfig := proxy.ProvideRuntimeConfig(configConfig)
	runtime := proxy.ProvideRuntime(runtimeConfig)
	nodeCheckCache := proxy.ProvideNodeCheckCache(redisClient)
	proxyService := proxy.ProvideService(proxySQLRepository, proxySecretCipher, subscriptionNormalizer, runtime, runtimeConfig, nodeCheckCache)
	registrationProcessor := sitediscovery.ProvideRegistrationProcessor(sitediscoverySQLRepository, suppliersService, sitediscoveryCredentialCipher, recorder, proxyService)
	...
	extensionService := extension.ProvideService(extensionSQLRepository, ingestProcessor, suppliersService, recorder)
	puritySQLRepository := purity.NewSQLRepository(db)
	purityService := purity.ProvideService(puritySQLRepository, accountRepository)
	schedulerService := scheduler.ProvideService(schedulerSQLRepository, suppliersService, extensionService, suppliergroupsService, ratesService, balancesService, healthService, usagecostsService, costsService, channelchecksService, purityService, sessionsService)
	...
	mailverificationSecretCipher := mailverification.UseSecretCipher(secretEncryptor)
	mailverificationSQLRepository := mailverification.NewSQLRepositoryWithCipher(db, mailverificationSecretCipher)
	...
	emailSender := adminplus2.UseMailVerificationEmailSender(emailService)
	mailverificationService := mailverification.ProvideService(mailverificationSQLRepository, gmailProvider, defaultTemplateClassifier, emailSender, recorder)
	registrationMailReader := sitediscovery.UseRegistrationMailReader(mailverificationService)
	registrationLogReader := sitediscovery.UseRegistrationLogReader(opsService)
	sitediscoveryService := sitediscovery.ProvideService(sitediscoverySQLRepository, suppliersService, extensionService, registrationMailReader, router, sitediscoveryCredentialCipher, httpClient, recorder, registrationLogReader, proxyService)
	...
	importexportService := importexport.NewService(db)
	importExportHandler := adminplus.NewImportExportHandler(importexportService)
	adminPlusHandlers := handler.ProvideAdminPlusHandlers(supplierHandler, supplierGroupHandler, supplierKeyHandler, provisionJobHandler, rateHandler, balanceHandler, healthHandler, kanbanHandler, notificationHandler, usageCostHandler, costHandler, channelCheckHandler, accountRateSyncHandler, extensionHandler, siteDiscoveryHandler, siteCatalogHandler, publicProxyAIHandler, purityHandler, mailVerificationHandler, sessionHandler, schedulerHandler, actionHandler, sub2APIHandler, proxyHandler, backupHandler, importExportHandler)
