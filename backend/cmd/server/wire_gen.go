...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	notificationSecretCipher := notifications.UseSecretCipher(secretEncryptor)
	notificationService := notifications.NewServiceWithCipher(notificationsSQLRepository, notificationSecretCipher)
	feishuNotifier := rates.NewFeishuNotifier(notificationService)
	...
	balancesFeishuNotifier := balances.NewFeishuNotifier(notificationService)
	...
	costsFeishuNotifier := costs.NewFeishuNotifier(notificationService)
	costsService := costs.NewServiceWithDependenciesAndNotifier(costsSQLRepository, costsFeishuNotifier, sessionsService, router, router, usageCostService, balancesService, suppliersService)
	announcementsSQLRepository := announcements.NewSQLRepository(db)
	announcementsFeishuNotifier := announcements.NewFeishuNotifier(notificationService)
	...
	healthFeishuNotifier := health.NewFeishuNotifier(notificationService)
	...
	notificationHandler := adminplus.NewNotificationHandler(notificationService)
	...
	schedulerService := scheduler.ProvideService(schedulerSQLRepository, suppliersService, extensionService, suppliergroupsService, ratesService, balancesService, announcementsService, healthService, usageCostService, channelchecksService, sessionsService)
