...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	identityRepository := sub2api.NewIdentityRepository(readDB)
	...
	authService := service.ProvideAdminPlusAuthService(client, userRepository, identityRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService)
	...
	authHandler := handler.NewAuthHandler(configConfig, authService, settingService, totpService)
	...
	healthSQLRepository := health.NewSQLRepositoryWithReadDB(db, readDB)
	...
	registrationProcessor := sitediscovery.ProvideRegistrationProcessor(sitediscoverySQLRepository, suppliersService, sitediscoveryCredentialCipher, recorder)
	...
	sitediscoveryService := sitediscovery.ProvideService(sitediscoverySQLRepository, suppliersService, extensionService, router, sitediscoveryCredentialCipher, httpClient, recorder, registrationLogReader)
	...
	opsScheduledReportService := service.ProvideOpsScheduledReportService(opsService, identityRepository, emailService, redisClient, configConfig)
