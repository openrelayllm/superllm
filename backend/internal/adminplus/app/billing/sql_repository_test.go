...
func newBillingSQLMock(t *testing.T) (*sql.DB, sqlmock.Sqlmock) {
	t.Helper()
...
func TestSQLRepositoryCreateBillLine(t *testing.T) {
	db, mock := newBillingSQLMock(t)
	...
	mock.ExpectQuery(`INSERT INTO admin_plus_supplier_bill_lines`).
		WithArgs(
			int64(7),
			"chrome",
			"bill-1",
		...
		WillReturnRows(newBillLineRows().AddRow(
			int64(11),
			int64(7),
			"chrome",
			"bill-1",
	...
	got, err := repo.CreateBillLine(context.Background(), &adminplusdomain.SupplierBillLine{
		SupplierID:        7,
		Source:            "chrome",
...
func newBillLineRows() *sqlmock.Rows {
