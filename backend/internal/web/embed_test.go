...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		...
		assert.Contains(t, string(result), "<title>MyCustomSite - Operations Automation Console</title>")
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		...
		assert.Contains(t, string(result), "<title>TestSite - Operations Automation Console</title>")
	})
}
