...
// legacyDeps 用于兼容旧测试调用签名，运行时会通过它注入 TurnstileService。
func NewSettingHandler(settingService *service.SettingService, opsService *service.OpsService, legacyDeps ...any) *SettingHandler {
	...
	for _, dep := range legacyDeps {
		if turnstileService, ok := dep.(*service.TurnstileService); ok {
			handler.turnstileService = turnstileService
		}
	}
	return handler
}
...
func (h *SettingHandler) GetSettings(c *gin.Context) {
	...
	payload := dto.SystemSettings{
		...
		ProxyAIPurityTurnstileEnabled:             settings.ProxyAIPurityTurnstileEnabled,
		ProxyAIPurityTurnstileSiteKey:             settings.ProxyAIPurityTurnstileSiteKey,
		ProxyAIPurityTurnstileSecretKeyConfigured: settings.ProxyAIPurityTurnstileSecretKeyConfigured,
...
type UpdateSettingsRequest struct {
	...
	ProxyAIPurityTurnstileEnabled   bool   `json:"proxyai_purity_turnstile_enabled"`
	ProxyAIPurityTurnstileSiteKey   string `json:"proxyai_purity_turnstile_site_key"`
	ProxyAIPurityTurnstileSecretKey string `json:"proxyai_purity_turnstile_secret_key"`
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	req.ProxyAIPurityTurnstileSiteKey = strings.TrimSpace(req.ProxyAIPurityTurnstileSiteKey)
	req.ProxyAIPurityTurnstileSecretKey = strings.TrimSpace(req.ProxyAIPurityTurnstileSecretKey)
	if req.ProxyAIPurityTurnstileEnabled {
		if req.ProxyAIPurityTurnstileSiteKey == "" {
			response.BadRequest(c, "ProxyAI purity Turnstile Site Key is required when enabled")
			return
		}
		if req.ProxyAIPurityTurnstileSecretKey == "" {
			if previousSettings.ProxyAIPurityTurnstileSecretKey == "" {
				response.BadRequest(c, "ProxyAI purity Turnstile Secret Key is required when enabled")
				return
			}
			req.ProxyAIPurityTurnstileSecretKey = previousSettings.ProxyAIPurityTurnstileSecretKey
		}
		siteKeyChanged := previousSettings.ProxyAIPurityTurnstileSiteKey != req.ProxyAIPurityTurnstileSiteKey
		secretKeyChanged := previousSettings.ProxyAIPurityTurnstileSecretKey != req.ProxyAIPurityTurnstileSecretKey
		if siteKeyChanged || secretKeyChanged {
			if h.turnstileService == nil {
				response.BadRequest(c, "Turnstile service is not configured")
				return
			}
			if err := h.turnstileService.ValidateSecretKey(c.Request.Context(), req.ProxyAIPurityTurnstileSecretKey); err != nil {
				response.ErrorFrom(c, err)
				return
			}
		}
	...
	settings := &service.SystemSettings{
		...
		ProxyAIPurityTurnstileEnabled:    req.ProxyAIPurityTurnstileEnabled,
		ProxyAIPurityTurnstileSiteKey:    req.ProxyAIPurityTurnstileSiteKey,
		ProxyAIPurityTurnstileSecretKey:  req.ProxyAIPurityTurnstileSecretKey,
	...
	payload := dto.SystemSettings{
		...
		ProxyAIPurityTurnstileEnabled:             updatedSettings.ProxyAIPurityTurnstileEnabled,
		ProxyAIPurityTurnstileSiteKey:             updatedSettings.ProxyAIPurityTurnstileSiteKey,
		ProxyAIPurityTurnstileSecretKeyConfigured: updatedSettings.ProxyAIPurityTurnstileSecretKeyConfigured,
